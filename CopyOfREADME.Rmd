...
> Examples of application of RNN using MXNetR can be [found here](https://jeremiedb.github.io/mxnet_R_bucketing/)
...
symbol_single <- rnn.graph(config = "seq-to-one", cell_type = "lstm", 
                           num_rnn_layer = 1, num_embed = 4, num_hidden = 12, 
...
```{r logger2, echo = FALSE, eval = TRUE, fig.height=3}
p <- plot_ly(x = seq_len(length(logger$train)), y = logger$train, 
             type = "scatter", mode = "markers+lines", name = "train", width = 600, height = 400) %>% 
  add_trace(y = logger$eval, type = "scatter", mode = "markers+lines", name = "eval") %>% 
  layout(font = list(size=20))
