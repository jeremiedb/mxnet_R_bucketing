...
```{r, echo=T}
source("mx.io.bucket.iter.no_mask.R")
source("rnn.unroll.R")
source("rnn.R")
source("rnn.train.R")
```
...
train.data <- mx.io.bucket.iter(buckets = corpus_bucketed_train$buckets, batch.size = batch.size, shuffle = TRUE)

eval.data <- mx.io.bucket.iter(buckets = corpus_bucketed_test$buckets, batch.size = batch.size, shuffle = FALSE)
...
initializer <- mx.init.Xavier(rnd_type = "gaussian", factor_type = "avg", magnitude = 1.5)

# optimizer <- mx.opt.create("adadelta", rho=0.92, epsilon=1e-6, wd=1e-5, clip_gradient=NULL, rescale.grad=1/batch.size)

optimizer <- mx.opt.create("rmsprop", learning.rate = 0.002, gamma1 = 0.95, gamma2 = 0.9, wd = 1e-5, clip_gradient=NULL, rescale.grad=1/batch.size)
