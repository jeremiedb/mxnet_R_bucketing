...
rnn_graph<- rnn.unroll(seq.len=2,
                       num.rnn.layer = 1,
                       num.hidden = 10,
                       input.size=500,
                       num.embed=8,
...
optimizer <- mx.opt.create("adadelta", rho=0.92, epsilon=1e-5, wd=1e-8, clip_gradient=NULL, rescale.grad=1/batch_size)

# optimizer <- mx.opt.create("adagrad", learning.rate=0.05, epsilon=1e-7, wd=1e-8, clip_gradient=NULL, rescale.grad=1/batch_size)

# optimizer <- mx.opt.create("rmsprop", learning.rate=0.002, gamma1=0.95, gamma2=0.90, wd=1e-6, clip_gradient=NULL, rescale.grad=1/batch_size)

# optimizer <- mx.opt.create("sgd", learning.rate=0.05, momentum=0.5, wd=1e-8, clip_gradient=NULL, rescale.grad=1/batch_size)

logger <- mx.metric.logger()
batch.end.callback <- mx.callback.log.train.metric(period = 50)
epoch.end.callback <- mx.callback.log.early.stop(period = 1, logger = logger, early_stop_rounds = 4, maximize = T)
...
mx.model.save(model_sentiment, prefix = "../models/model_kaggle_IMDB_GRU_1layer", iteration = 8)
...
### Inference
ctx <- list(mx.cpu())
model_sentiment <- mx.model.load(prefix = "../models/model_kaggle_IMDB_GRU_1layer", iteration = 8)
...
write_csv(submission, path = "../submit/kaggle_IMDB_GRU_1layer.csv")
