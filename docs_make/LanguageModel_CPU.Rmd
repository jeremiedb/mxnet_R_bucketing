...
graph.viz(rnn_graph, type = "graph", direction = "TD", 
          graph.height.px = 800, graph.width.px = 800, shape=list(data = c(2, 32), label = c(2, 32)))
```
...
# plotly::export(p, file = "logger_one_to_one_cpu.png")
p
```
...
infer <- mx.rnn.infer.unroll(data = data, 
...
for (i in 1:100) {
  ...
  infer <- mx.rnn.infer.unroll(data = pred, 
                               symbol = symbol.infer,
                               num_hidden = 80,
                               arg.params = model$arg.params,
                               aux.params = model$aux.params,
...
for (i in 1:100) {
  ...
  infer <- mx.rnn.infer.unroll(data = mx.nd.array(array(pred, dim = c(1,1))), 
                               symbol = symbol.infer,
                               num_hidden = 80,
                               arg.params = model$arg.params,
                               aux.params = model$aux.params,
