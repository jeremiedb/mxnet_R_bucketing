...
> This tutorial presents an example of application of RNN to text classification using padded and bucketed data to efficiently handle sequences of varying lengths. 
...
symbol_single <- rnn.graph.unroll(seq_len = 600, config = "seq-to-one", cell_type = "lstm", 
...
The utility function `mx.infer.rnn` has been added to simplify inference on RNN and support bucketed data. 
