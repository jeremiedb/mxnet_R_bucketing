...
seq_len <- 3
symbol_single <- rnn.graph.unroll(seq_len = seq_len, config = "seq-to-one", cell_type = "rich", 
...
symbol_single <- rnn.graph.unroll(seq_len = seq_len, config = "seq-to-one", cell_type = "rich", 
                                  num_rnn_layer = 1, num_embed = 2, num_hidden = 8, 
                                  num_decode = 2, input_size = vocab, dropout = 0.2, 
                                  ignore_label = -1, loss_output = "softmax",
                                  output_last_state = F, masking = T)
```
...
system.time(
  model <- mx.model.buckets(symbol = symbol_single,
                            ...
                            num.round = 4, ctx = devices, verbose = TRUE,
